...
func VisionOpenIAFullRetorno(ctx context.Context, llm, imageInput, apiKey, prompt, modelo, qualidadeImagem string) (interface{}, error) {
	// Verificação obrigatória dos parâmetros
	if imageInput == "" {
		return "", errors.New("imageInput não pode ser vazio")
	}
	...
	// Verificação simples para distinguir URL vs. Base64
	if strings.HasPrefix(imageInput, "http") {
		return uc.AnalyzeImageFromURLReturnComplete(ctx, imageInput, prompt, modelo, qualidadeImagem)
	}

	return uc.AnalyzeImageFromBase64ReturnComplete(ctx, imageInput, prompt, modelo, qualidadeImagem)
}
